...
class mrTransactionTrend(MRJob):
    def steps(self):
        return [
...
if __name__ == '__main__':
    mrTransactionTrend.run()
    df = pd.read_csv("mapreduce_transform/output/mrTransactionTrend.txt", delimiter="\t")
    df.to_csv("mapreduce_transform/output/mrTransactionTrend.csv", encoding='utf-8', index=False)
    os.remove("mapreduce_transform/output/mrTransactionTrend.txt")